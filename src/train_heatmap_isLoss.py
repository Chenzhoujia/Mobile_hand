        ...
        with tf.Session(config=config) as sess:
            ...
            for step in tqdm(range(total_step_num)):
                ...
                if step % params['per_update_tensorboard_step'] == 0:
                    ...
                    ax5.imshow(input_image_v[params['batchsize'], :, :, :])#第一个batch的维度 hand1(0~31) back1(32~63)
                    ax5.axis('off')
                    ax5.set_title(str(is_loss_v[params['batchsize']]))#hand1 back1
                    ...
                    ax6.imshow(scoremap_v[params['batchsize'], :, :, 0])  # 第一个batch的维度 hand1(0~31) back1(32~63)
                    ax6.axis('off')
                    ax6.set_title(str(is_loss_v[params['batchsize']]))  # hand1 back1
                    ...
                    ax7.imshow(preheat_v[params['batchsize'], :, :, 0])  # 第一个batch的维度 hand1(0~31) back1(32~63)
                    ax7.axis('off')
                    ax7.set_title(str(pre_is_loss_v[params['batchsize']]))  # hand1 back1

                    ax4 = fig.add_subplot(2, 4, 4)
                    ax4.imshow(pred_heatmaps_tmp_01_modi_v[0, :, :, 0])  # 第一个batch的维度 hand1(0~31) back1(32~63)
                    ax4.axis('off')
                    ax8 = fig.add_subplot(2, 4, 8)
                    ax8.imshow(pred_heatmaps_tmp_01_modi_v[params['batchsize'], :, :, 0])  # 第一个batch的维度 hand1(0~31) back1(32~63)
                    ax8.axis('off')
